...
The code gives the graph but does not allow rendering...

library(dplyr)
library(ggplot2)

# S'assurer que courses est chargé
if(!exists("courses")) {
  courses <- read.csv("courses.csv")  # Ajustez le chemin si nécessaire
}

# Trouver le cours et calculer en une seule étape
course_id_target <- courses %>%
  filter(course_name == "Moral Philosophy and Ethical Decision-Making") %>%
  pull(course_id)

# Vérifier que course_id_target n'est pas vide
if(length(course_id_target) == 0) {
  stop("Course not found!")
}

# Calculer les moyennes par groupe
avg_grades <- grades %>%
  filter(course_id == course_id_target[1], !is.na(grade)) %>%
...
```
The grades in trimester 3 are more dispersed and the trimester two has lower grades in average.
...
ggplot(rad_comm_avgs, aes(x = comm_avg, y = rad_avg)) +
  ...
  theme_minimal()
```
There is no clear relationship between those two courses' grades.
