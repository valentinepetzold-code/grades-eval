...
```{r}
# Visualiser final grade par group
...
# Ajouter l'effet du sexe (facetted)
...
```{r}
# Rappel: conditions pour passer

# - aucune moyenne de cours < 5

# - moyenne de chaque trimester >= 10 (la moyenne d'un trimestre = moyenne des moyennes des cours du trimestre)

# 1) Calculer la moyenne par étudiant et cours (déjà avg_by_student_course)

student_course_avg <- avg_by_student_course %>%
select(id, course_id, avg_grade) %>%
left_join(courses %>% select(course_id, trimester), by = "course_id")

# 2) Condition 1 : aucune moyenne de cours < 5

cond1 <- student_course_avg %>%
group_by(id) %>%
summarise(any_below_5 = any(avg_grade < 5, na.rm = TRUE))

# 3) Condition 2 : moyen par trimester >= 10
